...
export class OrdersQueueService {
  ...
  async resumeQueue(global = true): Promise<void> {
    ...
    this.logService.log(
      `[OrdersQueueService] Queue resumed (global=${global})`,
    );
  }

  async clean(
    state: 'wait' | 'active' | 'delayed' | 'completed' | 'failed',
    grace = 0,
  ): Promise<number> {
  ...
  async getJobs(
    types: Array<'waiting' | 'active' | 'delayed' | 'completed' | 'failed'> = [
      'waiting',
    ],
    start = 0,
    end = 50,
  ): Promise<Job[]> {
    const jobs = await this.ordersQueue.getJobs(types, start, end);
    return jobs.filter((j) => !!j);
  }

  async purgeAll(): Promise<void> {
    try {
      await this.clean('wait');
    } catch {}
    try {
      await this.clean('active');
    } catch {}
    try {
      await this.clean('delayed');
    } catch {}
    try {
      await this.clean('completed');
    } catch {}
    try {
      await this.clean('failed');
    } catch {}
