import { useState } from 'react';
import { api } from '@/lib/api';
import { QueueStatus } from '@/lib/types';

interface QueueErrorRecoveryProps {
  queueStatus?: QueueStatus;
  className?: string;
  onRecoveryAction?: () => void;
}

export default function QueueErrorRecovery({
  queueStatus,
  className = '',
  onRecoveryAction,
}: QueueErrorRecoveryProps) {
  const [isRetrying, setIsRetrying] = useState(false);
  const [retryResult, setRetryResult] = useState<string | null>(null);

  const hasFailed = (queueStatus?.failed || 0) > 0;
  
  async function handleRetryFailedJobs() {
    if (isRetrying) return;
    
    try {
      setIsRetrying(true);
      setRetryResult(null);
      
      const result = await api.retryFailed();
      
      setRetryResult(`${result.retried} jobs foram reprocessados com sucesso`);
      if (onRecoveryAction) onRecoveryAction();
    } catch (error) {
      setRetryResult(`Erro: ${(error as Error).message || 'Falha ao tentar reprocessar'}`);
    } finally {
      setIsRetrying(false);
    }
  }

  if (!hasFailed) return null;

  return (
    <div className={`rounded-lg bg-amber-50 border border-amber-200 p-4 ${className}`}>
      <div className="flex flex-col gap-4">
        <div className="flex items-center">
          <div className="flex-shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-amber-600" viewBox="0 0 20 20" fill="currentColor">
              <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
            </svg>
          </div>
          <div className="ml-3">
            <h3 className="text-sm font-medium text-amber-800">
              {queueStatus?.failed} jobs com falha detectados
            </h3>
            <div className="mt-2 text-sm text-amber-700">
              <p>
                Existem jobs que falharam no processamento. Você pode tentar reprocessá-los para resolver problemas de travamento.
              </p>
            </div>
          </div>
        </div>
        
        <div className="flex justify-end gap-2">
          <button
            type="button"
            disabled={isRetrying}
            onClick={handleRetryFailedJobs}
            className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-amber-600 hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500 disabled:opacity-50"
          >
            {isRetrying ? 'Reprocessando...' : 'Reprocessar jobs com falha'}
          </button>
        </div>
        
        {retryResult && (
          <div className="mt-2 text-sm text-amber-700 bg-amber-100 p-2 rounded">
            {retryResult}
          </div>
        )}
      </div>
    </div>
  );
}